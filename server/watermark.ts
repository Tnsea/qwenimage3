...
export function watermarkedExport(asset: SourceAsset): ExportAsset {
  ...
  const patternWidth = Math.max(320, Math.round(fontSize * 16));
  ...
  <defs>
    <pattern id="qwen-free-watermark" width="${patternWidth}" height="${patternHeight}" patternUnits="userSpaceOnUse" patternTransform="rotate(-24)">
      <text x="0" y="${Math.round(patternHeight * 0.58)}" fill="#ffffff" fill-opacity="0.18" font-family="Inter,Arial,sans-serif" font-size="${fontSize}" font-weight="700" letter-spacing="${Math.max(1, Math.round(fontSize * 0.08))}">QWEN IMAGE HUB · FREE</text>
    </pattern>
  </defs>
  ...
  <g transform="translate(${badgeX} ${badgeY})">
    ...
    <text x="${Math.round(badgeHeight * 0.82)}" y="${Math.round(badgeHeight * 0.63)}" fill="#ffffff" font-family="Inter,Arial,sans-serif" font-size="${Math.max(14, Math.round(fontSize * 0.78))}" font-weight="650">Qwen Image Hub · Free export</text>
  </g>
</svg>`;
