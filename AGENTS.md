...
- Local development and the Pages fallback default to the deterministic SVG preview; the canonical acceptance Worker currently selects Kie.ai.
...
Keep Studio generation actions aligned with the direct-to-history handoff, push every acceptance deployment revision through CI/review, and verify the remaining blockers in `docs/RELEASE_READINESS.md`. Do not delete runtime data, generated artifacts, branches, or worktrees without an explicit post-report confirmation.
