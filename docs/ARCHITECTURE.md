...
- The exact deployed source state, Worker version, rollback identifiers, live-smoke evidence, and deferred gates are maintained only in [Release Readiness](./RELEASE_READINESS.md). The current Cloudflare acceptance Worker renders the complete Create conversation, publishes generation to standard/priority queues, polls owned status, and shares actions with History. A signed-in canonical browser observed `Submitting`, `Generating`, and in-place `Complete`; the 2048×2048 private image loaded, four credits settled, D1 retained the provider task ID/R2 key, and the browser console had no warning/error. Local tests cover the queued and terminal-failure/refund states. The canonical catalog and prerendered HTML omit the inactive local preview model, and the deterministic adapter remains selected only by local development and the separately deployed Pages fallback. Restored-binding promotion, Kie.ai failure/timeout behavior, and provider commercial approval remain open.
