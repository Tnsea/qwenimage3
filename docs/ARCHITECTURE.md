...
- Committed revision `551146d0e8fc3a333ed6dc9e883f135cb3e426c6` is deployed as Worker `a84277dd-c81d-4944-a4da-418aaec1f959`. The canonical Worker keeps new Checkout disabled while Stripe webhook settlement and external cleanup remain configured. The prior billing-disabled Worker `e9315acd-5b98-462d-b4ed-dcc7bc0845d8` is the safe code rollback point; immediate predecessor `534c16c2-610f-42ed-8c27-21447fdff467` enabled Checkout and is not a safe rollback without a new billing decision. Successful client generations hand off directly to private Studio history without inline result/recent galleries. Studio now uses persistent light/dark selection with a graphite/amber dark system; local desktop and 390 px browser acceptance covered Overview, Create, Settings, theme switching, overflow, and the corrected mobile verification alert. Canonical assets `/assets/index-DAuUAkK5.js` and `/assets/index-Cgqznlu8.css` contain the same theme and mobile-layout contracts. Studio history retains authenticated download, favorite, variation, failed-generation retry, and confirmed permanent removal controls. The canonical catalog and prerendered HTML omit the inactive local preview model, and the deterministic adapter remains selected only by local development and the separately deployed Pages fallback at `https://623d913e.qwen-image-3.pages.dev`. No new signed-in provider request was spent for this UI deployment. The Kie.ai Qwen Image 2 path previously completed both a direct Worker acceptance task and one signed-in product request with four-credit settlement, private R2 persistence, and browser rendering. Failure/timeout behavior and provider commercial approval remain open.
