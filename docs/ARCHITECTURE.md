...
- Committed account-required revision `3022d254ed2ed5e169278477d18485c4d4cf7899` is deployed as Worker `1a06f468-914d-454b-adad-a877a040c305`; prior Worker `538f9a37-184b-466d-b897-ca5b98c5038c` is the recorded code rollback point. Successful client generations hand off directly to private Studio history without inline result/recent galleries. Studio history retains authenticated download, favorite, variation, failed-generation retry, and confirmed permanent removal controls. The Pages fallback was rebuilt from the same source at immutable deployment `https://623d913e.qwen-image-3.pages.dev`. The duplicate Prompts page and its navigation, footer, canonical, and sitemap entries are removed; legacy `/prompts` requests permanently redirect to `/examples`. The deployed bundles and canonical routes passed read-only smoke, while a new signed-in generation was not spent for this UI-only deployment. The Kie.ai Qwen Image 2 path previously completed both a direct Worker acceptance task and one signed-in product request with four-credit settlement, private R2 persistence, and browser rendering. Paid Starter receives original, watermark-free exports while retaining the standard queue; Creator and Professional retain both original exports and VIP priority. Failure/timeout behavior and provider commercial approval remain open.
