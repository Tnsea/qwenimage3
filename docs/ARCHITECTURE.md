...
- Committed revision `b540e47cea24128697a3f9845297d4e2cde14a77` is deployed as Worker `e5380627-b98e-45f5-bd77-028baaaa61e5`. The canonical Worker enables new Live-mode Checkout by explicit repository-owner decision while Stripe webhook settlement and external cleanup remain configured. Immediate predecessor `a84277dd-c81d-4944-a4da-418aaec1f959` is the billing-disabled rollback for shutting off new Checkout; D1 restore remains unexercised. Successful client generations hand off directly to private Studio history without inline result/recent galleries. Studio uses persistent light/dark selection with a graphite/amber dark system; local desktop and 390 px browser acceptance covered Overview, Create, Settings, theme switching, overflow, and the corrected mobile verification alert. Canonical assets `/assets/index-DAuUAkK5.js` and `/assets/index-Cgqznlu8.css` contain the same theme and mobile-layout contracts. Studio history retains authenticated download, favorite, variation, failed-generation retry, and confirmed permanent removal controls. The canonical catalog and prerendered HTML omit the inactive local preview model, and the deterministic adapter remains selected only by local development and the separately deployed Pages fallback at `https://623d913e.qwen-image-3.pages.dev`. No signed-in Checkout or new provider request was spent for this configuration deployment. The Kie.ai Qwen Image 2 path previously completed both a direct Worker acceptance task and one signed-in product request with four-credit settlement, private R2 persistence, and browser rendering. Failure/timeout behavior and provider commercial approval remain open.
