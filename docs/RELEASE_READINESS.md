...
- Source revision and CI: committed revision `6b88386988480be5f4150c56987e444e1dd668ae`; local release verification passed during the Software Bolt badge deployment with 102 tests, production artifact scanning, and zero production dependency findings. This local commit has not been pushed and has no matching GitHub CI run; historical run `30062662075` applies only to earlier revision `dd38e44`
- Worker version: `2f1abf93-ff83-4923-9c12-e233896a5bd0`
...
- Live smoke: July 24, 2026, 17:25 Asia/Shanghai; canonical health reported `ok`, Worker `2f1abf93-ff83-4923-9c12-e233896a5bd0`, D1/R2, fresh maintenance, Kie.ai selected/configured, billing enabled, configured Stripe credentials/webhook/catalog, zero failed/stale events, zero open/unrecovered reviews, and healthy alerting. Canonical HTML loaded `/assets/index-ZUPTRQdD.js` and `/assets/index-Tyh0kHIg.css`, placed Software Bolt below Findly.tools in the vertical footer recognition group, contained no Startup Fame markup, and retained one canonical link. The Software Bolt image returned HTTP 200 as a 203 × 54 PNG. The response CSP permits the exact `findly.tools` and `softwarebolt.com` image hosts; neither appears in `script-src` or `connect-src`. No canonical signed-in Checkout, payment, or provider request was used for this presentation deployment, so the prior Sandbox lifecycle and signed-in generation evidence remains authoritative. The Pages fallback was not redeployed.
