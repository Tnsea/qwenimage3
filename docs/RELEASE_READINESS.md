...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `b95303e` runs as Worker `7bfce1bd-6785-4e60-a30e-610ea5346ba1`; health reports configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. |
...
| `OPS-002` | Cloudflare custom-domain acceptance runtime serves committed source revision `b95303e` as Worker `7bfce1bd-6785-4e60-a30e-610ea5346ba1`; a pre-migration D1 export, forward migrations, alert evidence, and prior Worker identifier are recorded. Local release verification passed, while GitHub CI for this revision and restore/rollback execution remain unverified | CI evidence, health supervision, backup/restore, and rollback drill |
...
- Source revision and CI: committed revision `b95303e8bf003f18847c4502fa3f7b6b7b0611c5`; local `verify:release` passed with 86 tests before the GA4 follow-up, while GitHub CI for this revision remains pending
- Worker version: `7bfce1bd-6785-4e60-a30e-610ea5346ba1`
...
The Worker and client runtime content are represented by committed revision `b95303e` and were deployed only after local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision. The consent-gated GA4 follow-up remains a separate, not-yet-deployed change until its own verification and commit complete.
