...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `8007fa9` runs as Worker `d141caeb-4703-45a6-bb1d-53981c829fe8`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
| `OPS-002` | Cloudflare custom-domain acceptance runtime serves committed source revision `8007fa98d64cf5ff4f888fdc435431af29e49ffb` as Worker `d141caeb-4703-45a6-bb1d-53981c829fe8`; a pre-migration D1 export, forward migrations, alert evidence, GA4 installation evidence, and prior Worker identifiers are recorded. Immutable local release verification passed, while GitHub CI for this revision and restore/rollback execution remain unverified | CI evidence, health supervision, backup/restore, and rollback drill |
...
- Source revision and CI: committed revision `8007fa98d64cf5ff4f888fdc435431af29e49ffb`; immutable local `verify:release` passed with 92 tests and a zero-finding production dependency audit, while GitHub CI for this revision remains pending
- Worker version: `d141caeb-4703-45a6-bb1d-53981c829fe8`
...
- Rollback identifiers: prior reviewed Worker `efdc525d-46ee-4f54-bccb-eea021869fec`; D1 restore has not been exercised
- Live smoke: July 24, 2026, Asia/Shanghai; health reported `ok`, Worker `d141caeb-4703-45a6-bb1d-53981c829fe8`, D1/R2, fresh maintenance, Kie.ai selected/configured, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. A no-spend Kie readiness probe succeeded, then fixed task `1c72e61aba62832596065a6d4b0aeb48` completed and downloaded a validated PNG from the approved result host; the temporary diagnostic routes were absent after the immutable final version was restored. The canonical policy routes matched the approved version. The operator test returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. The default-on follow-up showed no analytics dialog or footer choice and inserted exactly one reviewed GA4 script on the live root. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `8007fa98d64cf5ff4f888fdc435431af29e49ffb` and were deployed only after immutable local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision.
