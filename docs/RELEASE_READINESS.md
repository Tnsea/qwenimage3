...
- Source revision and CI: committed revision `551146d0e8fc3a333ed6dc9e883f135cb3e426c6`; immutable local `verify:release` passed repeatedly with 102 tests, production artifact scanning, and zero production/full dependency findings. This local commit has not been pushed and has no new GitHub CI run; historical run `30062662075` applies only to earlier revision `dd38e44`
- Worker version: `a84277dd-c81d-4944-a4da-418aaec1f959`
...
- Rollback identifiers: prior billing-disabled Worker `e9315acd-5b98-462d-b4ed-dcc7bc0845d8`; immediate predecessor `534c16c2-610f-42ed-8c27-21447fdff467` enabled Checkout and must not be selected as a rollback without a new explicit billing decision. D1 restore has not been exercised
- Live smoke: July 24, 2026, 11:37 Asia/Shanghai; canonical health reported `ok`, Worker `a84277dd-c81d-4944-a4da-418aaec1f959`, D1/R2, fresh maintenance, Kie.ai selected/configured, billing disabled, configured Stripe credentials/webhook/catalog, zero failed/stale events, zero open/unrecovered reviews, and healthy alerting. The public catalog marked all six subscription intervals and three credit packs unconfigured for Checkout. `/studio/overview` returned HTTP 200 with the expected security headers; `www` returned a permanent 308 to the apex. Canonical HTML loaded `/assets/index-DAuUAkK5.js` and `/assets/index-Cgqznlu8.css`; direct asset probes found both Studio theme labels, the pending-offer handoff, `--workspace-bg:#0a0e11`, and the corrected mobile `grid-auto-flow:row`/responsive columns. Local browser acceptance covered authenticated dark/light Overview, dark Create and Settings, and the fixed 390 px verification layout with no horizontal overflow or console warning/error. No canonical signed-in browser session or new provider request was used for this UI deployment, so the prior paid provider and signed-in generation evidence remains authoritative. The Pages fallback was not redeployed.
...
The Worker and client runtime content are represented by committed revision `551146d0e8fc3a333ed6dc9e883f135cb3e426c6` and were deployed only after immutable local release verification. A matching GitHub CI run and reviewed release marker remain pending. This record documents acceptance evidence only—not a Ready decision.
