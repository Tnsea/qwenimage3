...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `dd38e44` runs as Worker `e9315acd-5b98-462d-b4ed-dcc7bc0845d8`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. The canonical catalog and prerendered HTML omit the inactive local preview model; the deterministic adapter remains available only when the local or Pages fallback runtime explicitly selects it. The canonical and Pages fallback bundles retain the Studio-history handoff and authenticated download/favorite/variation/retry/delete controls. Canonical public routes retain the previously verified status and `/models` passed a fresh HTTP and browser-rendering smoke with five columns and no Best for field; `www` permanently redirects to the apex. Legacy `/prompts` permanently redirects to `/examples`, and the live sitemap no longer includes `/prompts`. The Pages fallback remains deployment `623d913e.qwen-image-3.pages.dev` with the modern catalog contract and deterministic local preview provider. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
- Source revision and CI: committed revision `dd38e44df4691b8c733d52c9da42f9a85784916b`; immutable local `verify:release` passed with 99 tests and a zero-finding production dependency audit; GitHub Actions run `30062662075` passed the full workflow
- Worker version: `e9315acd-5b98-462d-b4ed-dcc7bc0845d8`
...
The Worker and client runtime content are represented by committed revision `dd38e44df4691b8c733d52c9da42f9a85784916b` and were deployed only after immutable local release verification and a passing GitHub CI run. This record documents acceptance evidence only—not a Ready decision.
