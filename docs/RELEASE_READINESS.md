...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `efe7104` runs as Worker `9d3a16f1-807c-4414-bddd-b5d26b80073e`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. The canonical catalog and prerendered HTML omit the inactive local preview model; the deterministic adapter remains available only when the local or Pages fallback runtime explicitly selects it. The canonical and Pages fallback bundles retain the Studio-history handoff and authenticated download/favorite/variation/retry/delete controls. Canonical public routes retain the previously verified status and `/models` passed a fresh HTTP and browser-rendering smoke; `www` permanently redirects to the apex. Legacy `/prompts` permanently redirects to `/examples`, and the live sitemap no longer includes `/prompts`. The Pages fallback remains deployment `623d913e.qwen-image-3.pages.dev` with the modern catalog contract and deterministic local preview provider. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
- Studio action and cleanup commit `532215a`, Actions runtime commit `3022d25`, and canonical catalog cleanup commit `efe7104` are pushed on `codex/worker-online-hardening`;
- GitHub Actions run `30061584261` passed the full CI workflow with 99 tests and zero dependency-audit findings for pull request 1.
...
**Evidence:** baseline commit `39b9e3a` is pushed to `main` in `Tnsea/qwenimage3`. Draft pull request 1 tracks the hardening branch. Runtime revision `efe7104` passed GitHub Actions run `30061584261` with strict checks, 99 tests, Worker checking, the production build, local-artifact scanning, and a zero-finding production dependency audit before deployment.
...
- Source revision and CI: committed revision `efe71042f531e51db5d7d1a1bdfe3c9b11455d77`; immutable local `verify:release` passed with 99 tests and a zero-finding production dependency audit; GitHub Actions run `30061584261` passed the full workflow
- Worker version: `9d3a16f1-807c-4414-bddd-b5d26b80073e`
...
- Rollback identifiers: prior Worker `1a06f468-914d-454b-adad-a877a040c305`; D1 restore has not been exercised
- Live smoke: July 24, 2026, 10:28 Asia/Shanghai; canonical health reported `ok`, Worker `9d3a16f1-807c-4414-bddd-b5d26b80073e`, D1/R2, fresh maintenance, Kie.ai selected/configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. Canonical `/api/catalog` exposed Alibaba Qwen Image 2.0 Pro, the available Kie.ai `qwen2/text-to-image` model, and the Qwen Image 3 roadmap entry, with no `local-preview` provider or `local-qwen-preview` model. `/models` returned HTTP 200 with the expected security headers; its first-response HTML contained no Fast Preview, Development fallback, or Local preview copy. A real browser render showed exactly those three catalog rows and no local-preview text. The canonical bundle loaded asset `/assets/index-CPgJ3jvH.js`; the Pages fallback was not redeployed and retains its prior immutable deployment and explicitly deterministic local-preview provider. No new signed-in provider request was made for this UI-only deployment, so the prior fixed paid provider task `1c72e61aba62832596065a6d4b0aeb48` and signed-in product generation `9cc97bfb-5dfd-4d46-9815-d08ccec64510` remain the external evidence for Kie.ai execution, four-credit settlement, private R2 persistence, browser rendering, and authenticated download access. The immutable release test verified that an unsubscribed account receives a watermarked SVG, paid Starter receives the original PNG bytes with a `starter` export tier, and Creator receives the original bytes with a `vip` export tier; a live paid-Starter download remains unexercised while Checkout is disabled. The operator alert test previously returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `efe71042f531e51db5d7d1a1bdfe3c9b11455d77` and were deployed only after immutable local release verification and a passing GitHub CI run. This record documents acceptance evidence only—not a Ready decision.
