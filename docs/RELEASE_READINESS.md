...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `526db5a` runs as Worker `a49ac359-11ef-4747-912b-5fd35346f751`; health reports configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 was detected successfully by Google's installation test; the subsequent default-on collection change requires its own deployment record. |
