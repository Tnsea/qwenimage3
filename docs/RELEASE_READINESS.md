...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `5491732` runs as Worker `f45421d1-4d70-4b27-af74-0b3bdbe63884`; health reports configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
### MODEL-001: Kie.ai Qwen Image 2 adapter is locally verified; external execution pending

**Evidence:** the Worker implements the documented `qwen2/text-to-image` task contract, bounded polling, immediate private R2 ingestion, exact API/result-host enforcement, redirect revalidation, image MIME/signature/size checks, and safe error mapping. The fixed 2K model is exposed only as a four-credit Standard generation with documented ratios and an 800-character prompt limit; both the client and server reject unsupported settings before provider spend. Catalog availability is fail-closed unless the exact model and complete Kie configuration are selected. Mocked integration tests cover one successful task, an unapproved result host, provider failure, and unsupported settings. A dedicated Kie key restricted to Qwen Image 2.0 was created on July 24, 2026, stored as a managed Worker secret, and capped at 200 hourly, 1,000 daily, and 4,000 lifetime Kie credits. Real generation acceptance remains pending.

**Impact:** provider-side model access, price, balance behavior, latency, moderation, result host, image dimensions, output quality, data/commercial terms, and late task completion have not been observed. The synchronous request may time out after Kie.ai has accepted a billable task, so public use could create untracked cost or refund product credits while provider spend continues.

**Acceptance:** create a dedicated restricted/spend-capped key, store it as a managed secret, run one paid signed-in generation in the Cloudflare acceptance environment, verify the exact returned host and immediate private R2 copy, reconcile Kie spend with the product ledger, test failure/timeout/refund behavior, record rollback, and approve the provider contract. Durable callback/queue handling or an explicitly accepted synchronous risk limit is required before production approval.
...
| `OPS-002` | Cloudflare custom-domain acceptance runtime serves committed source revision `5491732` as Worker `f45421d1-4d70-4b27-af74-0b3bdbe63884`; a pre-migration D1 export, forward migrations, alert evidence, GA4 installation evidence, and prior Worker identifiers are recorded. Local release verification passed, while GitHub CI for this revision and restore/rollback execution remain unverified | CI evidence, health supervision, backup/restore, and rollback drill |
...
- Source revision and CI: committed revision `5491732e98a0988de42caddfa461c4e7c978ee32`; local `verify:release` passed with 87 tests, while GitHub CI for this revision remains pending
- Worker version: `f45421d1-4d70-4b27-af74-0b3bdbe63884`
- Environment and URL: Cloudflare acceptance, `https://qwen-image-3.net`
- Enabled providers: deterministic local preview, Google OAuth published for external Google accounts, and Cloudflare Email Routing billing alerts; Live Stripe credentials/webhook/catalog are configured but new Checkout remains disabled; Qwen, GitHub, and transactional account email remain disabled
- Applied migrations: `0001`–`0013`; pre-change D1 export retained at ignored path `backups/qwen-image-3-20260723-155637.sql`
- Rollback identifiers: prior Worker `a49ac359-11ef-4747-912b-5fd35346f751`; D1 restore has not been exercised
- Live smoke: July 23, 2026, Asia/Shanghai; health reported D1/R2, fresh maintenance, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. The canonical policy routes matched the approved version. The operator test returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. The default-on follow-up showed no analytics dialog or footer choice and inserted exactly one reviewed GA4 script on the live root. Cloudflare's stale HTML cache was purged, after which the canonical root returned the current asset revision. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `5491732` and were deployed only after local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision.
