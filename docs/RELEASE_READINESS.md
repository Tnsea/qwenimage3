...
| Automated tests | 87 passed, 0 failed, including consent-gated GA4 page views, account-required generation, exactly-once welcome credits, 4/8/16 credit charging, Starter-versus-Creator entitlements, yearly-default pricing, explicit versioned policy acceptance, idempotent alert test delivery, alert deduplication/reminder/recovery, per-route self-referencing canonical metadata, unsupported-model rejection, stable non-JSON edge errors, concurrent D1 credits/idempotency, asynchronous Worker Stripe replay, out-of-order Radar fraud-warning retry/quarantine, authenticated and non-negative billing-review resolution, current Stripe Portal cancellation payloads, account-deletion billing tombstones, billing-event/review health degradation, maintenance repeatability, origin isolation, reviewed analytics CSP hosts, HTML transformation prevention, and external-policy contracts |
| Production bundle | Pass; JavaScript gzip about 98.7 KB; artifact scan rejects loopback addresses, SQLite customer copy, and development-token copy |
...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `526db5a` runs as Worker `a49ac359-11ef-4747-912b-5fd35346f751`; health reports configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. Consent-gated GA4 was detected successfully by Google's installation test. |
...
| `OPS-002` | Cloudflare custom-domain acceptance runtime serves committed source revision `526db5a` as Worker `a49ac359-11ef-4747-912b-5fd35346f751`; a pre-migration D1 export, forward migrations, alert evidence, GA4 installation evidence, and prior Worker identifiers are recorded. Local release verification passed, while GitHub CI for this revision and restore/rollback execution remain unverified | CI evidence, health supervision, backup/restore, and rollback drill |
...
- Source revision and CI: committed revision `526db5ab777c54ccac9f5092883c5a91ae8f6b39`; local `verify:release` passed with 87 tests, while GitHub CI for this revision remains pending
- Worker version: `a49ac359-11ef-4747-912b-5fd35346f751`
...
- Rollback identifiers: prior Worker `04d02c4e-4843-4e6c-b2a4-6607079c872e`; D1 restore has not been exercised
- Live smoke: July 23, 2026, Asia/Shanghai; health reported D1/R2, fresh maintenance, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. The canonical policy routes matched the approved version. The operator test returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. The live client showed the optional analytics choice before loading GA4, accepted the choice, and Google's setup screen then reported that the Google tag was detected correctly on the canonical site. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `526db5a` and were deployed only after local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision.
