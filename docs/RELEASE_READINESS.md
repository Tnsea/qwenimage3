...
**Evidence:** baseline commit `39b9e3a` is pushed to `main` in `Tnsea/qwenimage3`. Draft pull request 1 tracks the hardening branch. Its last remote revision `54fbec7` passed GitHub Actions run `30059019958` with strict checks, 98 tests, Worker checking, the production build, local-artifact scanning, and the blocking production dependency audit. Newer acceptance commits are still local and require a fresh pushed CI result.
