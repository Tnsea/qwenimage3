...
| Automated tests | 92 passed, 0 failed, including Kie.ai task creation/polling/download, exact result-host enforcement, and unsupported-setting rejection; default-on GA4 page views with advertising signals disabled; account-required generation; exactly-once welcome credits; 4/8/16 credit charging for multi-tier providers; Starter-versus-Creator entitlements; yearly-default pricing; explicit versioned policy acceptance; idempotent alert test delivery; alert deduplication/reminder/recovery; per-route self-referencing canonical metadata; unsupported-model rejection; stable non-JSON edge errors; concurrent D1 credits/idempotency; asynchronous Worker Stripe replay; out-of-order Radar fraud-warning retry/quarantine; authenticated and non-negative billing-review resolution; current Stripe Portal cancellation payloads; account-deletion billing tombstones; billing-event/review health degradation; maintenance repeatability; origin isolation; reviewed analytics CSP hosts; HTML transformation prevention; and external-policy contracts |
| Production bundle | Pass; JavaScript gzip about 98.1 KB; artifact scan rejects loopback addresses, SQLite customer copy, and development-token copy |
