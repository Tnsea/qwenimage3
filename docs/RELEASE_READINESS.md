...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `8e29524` runs as Worker `287d18a3-4feb-4961-bd02-55ab42effedb`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. The live root and Studio history route return HTTP 200, `www` redirects permanently to the apex domain, and the deployed bundle contains the Studio-history handoff without the removed inline result/recent classes. Canonical public routes, including `/pricing`, `/privacy`, `/status`, and `/support`, previously passed HTTP 200 and self-referencing-canonical checks, and the live sitemap includes each route. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
- Source revision and CI: committed revision `8e295245aa38ef5aa9dadcbd443c9969766bd361`; immutable local `verify:release` passed with 98 tests and a zero-finding production dependency audit, while GitHub CI for this revision remains pending
- Worker version: `287d18a3-4feb-4961-bd02-55ab42effedb`
...
- Rollback identifiers: prior reviewed Worker `9e63c7c1-1b9e-4c78-a608-5853dbfadbdd`; D1 restore has not been exercised
- Live smoke: July 24, 2026, 09:31 Asia/Shanghai; health reported `ok`, Worker `287d18a3-4feb-4961-bd02-55ab42effedb`, D1/R2, fresh maintenance, Kie.ai selected/configured, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. The root, anonymous session, catalog, and Studio history requests returned HTTP 200; `www` returned the expected 308 apex redirect. The deployed JavaScript bundle contains `/studio/history` and no `creation-output` or `creation-recent` classes. No new signed-in provider request was made for this UI-only deployment, so the prior fixed paid provider task `1c72e61aba62832596065a6d4b0aeb48` and signed-in product generation `9cc97bfb-5dfd-4d46-9815-d08ccec64510` remain the external evidence for Kie.ai execution, four-credit settlement, private R2 persistence, browser rendering, and authenticated download access. The immutable release test verified that an unsubscribed account receives a watermarked SVG, paid Starter receives the original PNG bytes with a `starter` export tier, and Creator receives the original bytes with a `vip` export tier; a live paid-Starter download remains unexercised while Checkout is disabled. The live status bundle contains the reviewed acceptance wording. The canonical policy routes matched the approved version. The operator alert test previously returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `8e295245aa38ef5aa9dadcbd443c9969766bd361` and were deployed only after immutable local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision.
