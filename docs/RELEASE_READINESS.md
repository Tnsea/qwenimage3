...
**Remaining:** enable Email Routing, verify the destination, deploy the binding/secrets, and receive one real test email. Delayed payment is not reachable while Checkout is restricted to synchronous `card` and `link`; the shared asynchronous-success handler has signed-event coverage.
...
**Remaining:** receive the real external alert email and verify the approved copy on the deployed canonical origin. The non-negative partial-recovery path is covered by the Worker integration suite; a future real partial Stripe refund should be added to external acceptance before relying on partial refunds operationally. Do not enable public billing from unreviewed source.
