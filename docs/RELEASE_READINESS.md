...
| Browser smoke | The current local build visually passes desktop and 390 px checks for the shared three-plan pricing interface plus `/privacy`, `/status`, and `/support`, with no horizontal overflow or browser console errors. Mobile navigation, authentication dialog, unauthenticated Studio gate, generation failure handling, and 404 were visually verified on the previous acceptance revision. The current account-required revision completed one signed-in Kie.ai generation in the browser, including the rendered private result, Save, Variation, authenticated Download, and a visible 20-to-16 credit change. |
...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `1734c5e` runs as Worker `538f9a37-184b-466d-b897-ca5b98c5038c`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. The live `/examples` route returns HTTP 200 with a self-referencing canonical, legacy `/prompts` returns a permanent redirect to `/examples`, and the live sitemap no longer includes `/prompts`. The deployed bundle retains the Studio-history handoff without the removed inline result/recent classes. Canonical public routes, including `/pricing`, `/privacy`, `/status`, and `/support`, previously passed HTTP 200 and self-referencing-canonical checks. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
| `OPS-002` | Cloudflare custom-domain acceptance runtime serves committed source revision `67155fc8fdb8769e1e1babde61887e0431aed668` as Worker `9e63c7c1-1b9e-4c78-a608-5853dbfadbdd`; a pre-migration D1 export, forward migrations, alert evidence, GA4 installation evidence, and prior Worker identifiers are recorded. Immutable local release verification passed, while GitHub CI for this revision and restore/rollback execution remain unverified | CI evidence, health supervision, backup/restore, and rollback drill |
...
| `WEB-001` | Client/API routing and the previous guest-enabled revision passed broad live smoke. The current account-gated revision passed canonical health/session/catalog, signed-in pricing, yearly-default/monthly-toggle, Checkout-disabled, redirect checks, and one signed-in Studio generation with private result rendering | Complete the browser matrix, error routes, and rollback acceptance |
...
- Source revision and CI: committed revision `1734c5e47b6b350f38cc61dd6be44ae2e472351f`; immutable local `verify:release` passed with 98 tests and a zero-finding production dependency audit, while GitHub CI for this revision remains pending
- Worker version: `538f9a37-184b-466d-b897-ca5b98c5038c`
...
- Rollback identifiers: prior Worker `287d18a3-4feb-4961-bd02-55ab42effedb`; D1 restore has not been exercised
- Live smoke: July 24, 2026, 09:50 Asia/Shanghai; health reported `ok`, Worker `538f9a37-184b-466d-b897-ca5b98c5038c`, D1/R2, fresh maintenance, Kie.ai selected/configured, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. `/examples` returned HTTP 200 with its own canonical; `/prompts?source=legacy` returned HTTP 308 to `/examples?source=legacy`; the live navigation, footer, and sitemap contain no Prompts page entry. The previously verified root, anonymous session, catalog, Studio history, and `www` apex redirect remain historical evidence from the prior UI deployment. No new signed-in provider request was made for this UI-only deployment, so the prior fixed paid provider task `1c72e61aba62832596065a6d4b0aeb48` and signed-in product generation `9cc97bfb-5dfd-4d46-9815-d08ccec64510` remain the external evidence for Kie.ai execution, four-credit settlement, private R2 persistence, browser rendering, and authenticated download access. The immutable release test verified that an unsubscribed account receives a watermarked SVG, paid Starter receives the original PNG bytes with a `starter` export tier, and Creator receives the original bytes with a `vip` export tier; a live paid-Starter download remains unexercised while Checkout is disabled. The live status bundle contains the reviewed acceptance wording. The canonical policy routes matched the approved version. The operator alert test previously returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `1734c5e47b6b350f38cc61dd6be44ae2e472351f` and were deployed only after immutable local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision.
