...
| Browser smoke | The current local build visually passes desktop and 390 px checks for the shared three-plan pricing interface plus `/privacy`, `/status`, and `/support`, with no horizontal overflow or browser console errors. Mobile navigation, authentication dialog, unauthenticated Studio gate, generation failure handling, and 404 were visually verified on the previous acceptance revision; the account-required generation flow still needs a fresh post-deploy browser pass. |
...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `2b50e14` runs as Worker `d36d2ba1-c241-4a14-9ec1-dfa0da9e8fa3`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. Canonical public routes, including `/pricing`, `/privacy`, `/status`, and `/support`, return HTTP 200 with self-referencing canonical links, and the live sitemap includes each route. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
- real Resend, GitHub, and Alibaba Qwen credentials were not exercised; Kie.ai completed one real Worker-side Qwen Image 2 task, but the signed-in product-credit/private-R2 lifecycle remains open; Google completed one successful acceptance sign-in but its denial/failure paths remain open; Stripe delayed payment methods are disabled, so no real `checkout.session.async_payment_succeeded` event can be generated by the current `card`/`link` Checkout contract;
...
| `OPS-002` | Cloudflare custom-domain acceptance runtime serves committed source revision `2b50e1411f23285587374200c0881ef2a96376aa` as Worker `d36d2ba1-c241-4a14-9ec1-dfa0da9e8fa3`; a pre-migration D1 export, forward migrations, alert evidence, GA4 installation evidence, and prior Worker identifiers are recorded. Immutable local release verification passed, while GitHub CI for this revision and restore/rollback execution remain unverified | CI evidence, health supervision, backup/restore, and rollback drill |
...
- Source revision and CI: committed revision `2b50e1411f23285587374200c0881ef2a96376aa`; immutable local `verify:release` passed with 97 tests and a zero-finding production dependency audit, while GitHub CI for this revision remains pending
- Worker version: `d36d2ba1-c241-4a14-9ec1-dfa0da9e8fa3`
...
- Rollback identifiers: prior reviewed Worker `d141caeb-4703-45a6-bb1d-53981c829fe8`; D1 restore has not been exercised
- Live smoke: July 24, 2026, Asia/Shanghai; health reported `ok`, Worker `d36d2ba1-c241-4a14-9ec1-dfa0da9e8fa3`, D1/R2, fresh maintenance, Kie.ai selected/configured, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. The homepage, `/pricing`, `/privacy`, `/status`, and `/support` returned HTTP 200 with self-referencing canonical links; the live sitemap exposed the new trust routes and robots.txt referenced that sitemap. The earlier no-spend Kie readiness probe and fixed paid task `1c72e61aba62832596065a6d4b0aeb48` remain acceptance evidence for the unchanged provider path. The canonical policy routes matched the approved version. The operator test returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. The default-on follow-up showed no analytics dialog or footer choice and inserted exactly one reviewed GA4 script on the live root. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `2b50e1411f23285587374200c0881ef2a96376aa` and were deployed only after immutable local release verification. The deployment was not produced by GitHub CI, and this record documents acceptance evidence only—not a Ready decision.
