...
| Production bundle | Pass; JavaScript gzip 101.50 KB; artifact scan rejects loopback addresses, SQLite customer copy, and development-token copy |
...
- Source revision and CI: committed revision `b1a2127ca88aff8bbc5d1d9bf6d8684a72bd65f4`; local release verification passed during the Startup Fame removal deployment with 102 tests, production artifact scanning, and zero production dependency findings. This local commit has not been pushed and has no matching GitHub CI run; historical run `30062662075` applies only to earlier revision `dd38e44`
- Worker version: `95cad010-81e2-4b7c-8d1a-9542a8aee7e8`
...
- Live smoke: July 24, 2026, 16:48 Asia/Shanghai; canonical health reported `ok`, Worker `95cad010-81e2-4b7c-8d1a-9542a8aee7e8`, D1/R2, fresh maintenance, Kie.ai selected/configured, billing enabled, configured Stripe credentials/webhook/catalog, zero failed/stale events, zero open/unrecovered reviews, and healthy alerting. Canonical HTML loaded `/assets/index-LBvSVace.js` and `/assets/index-B0OVqy5l.css`, retained the Findly.tools recognition badge after the footer copyright row, and contained no Startup Fame markup. The former Startup Fame verification path now resolves through the SPA fallback as HTML instead of serving the removed token. The response CSP permits only `findly.tools` for the remaining recognition image and contains no Startup Fame host; Findly.tools appears in neither `script-src` nor `connect-src`. No canonical signed-in Checkout, payment, or provider request was used for this presentation deployment, so the prior Sandbox lifecycle and signed-in generation evidence remains authoritative. The Pages fallback was not redeployed.
