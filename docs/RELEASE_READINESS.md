...
- Source revision and CI: committed revision `2d15895d7ffe1bf73ff7596268b8cd132bdfacec`; local release verification passed during the badge/CSP deployment with 102 tests, production artifact scanning, and zero production dependency findings. The final footer-only placement then passed strict checks, focused SEO/trust tests, and the production build before deployment. This local commit has not been pushed and has no matching GitHub CI run; historical run `30062662075` applies only to earlier revision `dd38e44`
- Worker version: `bce63359-a852-4208-9010-a52dbfe58221`
...
- Live smoke: July 24, 2026, 16:33 Asia/Shanghai; canonical health reported `ok`, Worker `bce63359-a852-4208-9010-a52dbfe58221`, D1/R2, fresh maintenance, Kie.ai selected/configured, billing enabled, configured Stripe credentials/webhook/catalog, zero failed/stale events, zero open/unrecovered reviews, and healthy alerting. Canonical HTML loaded `/assets/index-DZmHdeir.js` and `/assets/index-B6pVdmMn.css`, placed both recognition badges after the footer copyright row and outside the hero/generator, and exposed the existing Startup Fame verification token. Both remote badge images loaded at their natural dimensions with no browser warning/error. The response CSP added only the exact badge hosts to `img-src`; neither host appears in `script-src` or `connect-src`. No canonical signed-in Checkout, payment, or provider request was used for this presentation deployment, so the prior Sandbox lifecycle and signed-in generation evidence remains authoritative. The Pages fallback was not redeployed.
