...
- Qwen Image 3 editorial and SEO commit `c269ef4` is committed locally on `codex/worker-online-hardening` and deployed to acceptance as Worker `5aa794f5-38d6-4ea8-9ae3-005a4591864d`, but is not yet pushed or covered by a matching GitHub CI run;
...
**Evidence:** baseline commit `39b9e3a` is pushed to `main` in `Tnsea/qwenimage3`. Draft pull request 1 tracks the hardening branch. Runtime revision `dd38e44` passed GitHub Actions run `30062662075` with strict checks, 99 tests, Worker checking, the production build, local-artifact scanning, and a zero-finding production dependency audit before deployment. Current acceptance revision `c269ef4` passed the stricter local release command with 116 tests, production artifact scanning, and zero production dependency findings before deployment, then deployed as Worker `5aa794f5-38d6-4ea8-9ae3-005a4591864d`; the commit remains unpushed and has no corresponding GitHub CI run.
...
- Source revision and CI: committed revision `c269ef4c7e35a31a1f8092139408341af2e79cd2`; local release verification passed with 116 tests, production artifact scanning, and zero production dependency findings. This local commit has not been pushed and has no matching GitHub CI run; historical run `30062662075` applies only to earlier revision `dd38e44`
- Worker version: `5aa794f5-38d6-4ea8-9ae3-005a4591864d`
...
- Rollback identifiers: immediate predecessor `9d34b374-2964-419e-aad9-eb54a64dc772` is the pre-editorial acceptance Worker. Billing-disabled Worker `a84277dd-c81d-4944-a4da-418aaec1f959` remains the rollback for shutting off new Checkout, and earlier billing-disabled Worker `e9315acd-5b98-462d-b4ed-dcc7bc0845d8` remains in deployment history. D1 restore has not been exercised
- Live smoke: July 25, 2026, 02:24 Asia/Shanghai; canonical health reported `ok`, Worker `5aa794f5-38d6-4ea8-9ae3-005a4591864d`, D1/R2, fresh maintenance, Kie.ai selected/configured, billing enabled, configured Stripe credentials/webhook/catalog, zero failed/stale events, zero open/unrecovered reviews, and healthy alerting. Homepage, Guides, Blog, all four editorial routes, sitemap, unknown route, and public draft-preview route returned their expected 200/404 status, one-H1, title, canonical, and noindex contracts. Canonical HTML loaded `/assets/index-D27EBQnj.js` and `/assets/index-D-UpLrhi.css`. Google Search Console accepted and reread `https://qwen-image-3.net/sitemap.xml` on July 25 with status `Success` and 16 discovered pages; the Midjourney comparison URL was separately added to the priority crawl queue. A manual request for the step-by-step guide returned GSC's transient “try again later” error and was not retried because the successful sitemap already covers every published URL. No canonical signed-in Checkout, payment, or provider request was used for this content deployment, so the prior Sandbox lifecycle and signed-in generation evidence remains authoritative. The Pages fallback was not redeployed.
...
The Worker and client runtime content are represented by committed revision `c269ef4c7e35a31a1f8092139408341af2e79cd2`. A matching GitHub CI run and reviewed release marker remain pending. This record documents acceptance evidence, GSC submission, and the explicit Checkout operating decision only—not a Ready or production-approval decision.
