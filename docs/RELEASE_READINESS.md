...
- Create-conversation commits `1d6e731` and `33640c8` are pushed; GitHub Actions run `30154996131` passed for `33640c8`, while the deployed acceptance Worker still represents `1d6e731`;
...
**Evidence:** baseline commit `39b9e3a` is pushed to `main` in `Tnsea/qwenimage3`. Draft pull request 1 tracks the hardening branch. Runtime revision `dd38e44` passed GitHub Actions run `30062662075` with strict checks, 99 tests, Worker checking, the production build, local-artifact scanning, and a zero-finding production dependency audit before deployment. Create-conversation commit `1d6e731` deployed as Worker `15017717-b14a-42e3-8a9f-c7a4afa7c96a`; follow-up documentation commit `33640c8` is pushed and passed matching GitHub Actions run `30154996131`. The durable-queue revision passes the full local release command with 119 tests but does not yet have its own commit, CI run, or deployment evidence.
...
|---|---|---|
| `GEN-001` | Durable standard/priority queue submission, persisted provider task IDs, bounded transient retries, owned polling, terminal refund, stranded-state recovery, and R2 compensation pass locally | Canonical queue lifecycle, cancellation policy, late-completion reconciliation, queue/cost supervision |
...
- Source revision and CI: deployed revision `1d6e731eb0eab8ad8f41246fb160ae63f8ecff94`; follow-up commit `33640c8f7c6b8029599e36aa2ad90396573f749e` is pushed and passed GitHub Actions run `30154996131`. The durable-queue working tree passes local release verification with 119 tests, production artifact scanning, and zero production dependency findings, but is not represented by this deployment record until it is committed and deployed
...
- Rollback identifiers: immediate predecessor `5aa794f5-38d6-4ea8-9ae3-005a4591864d` is the pre-Create-conversation acceptance Worker. Billing-disabled Worker `a84277dd-c81d-4944-a4da-418aaec1f959` remains the rollback for shutting off new Checkout, and earlier billing-disabled Worker `e9315acd-5b98-462d-b4ed-dcc7bc0845d8` remains in deployment history. D1 export `backups/qwen-image-3-20260725-184500.sql` (SHA-256 `3c7b7650a2434b8a35c7caa7262883ba424f196f8b05ee3db75c206e70300050`) imported successfully into isolated rehearsal database `cf8c1fed-2db2-43ac-bb71-b3d7499d99b6`; promotion and Worker rollback have not been exercised
...
The current Worker and client runtime content are represented by committed revision `1d6e731eb0eab8ad8f41246fb160ae63f8ecff94`. The queued-generation revision remains a locally verified working-tree change until the deployment record is updated. Review, merge, and a release marker remain pending. This record documents acceptance evidence and the explicit Checkout operating decision only—not a Ready or production-approval decision.
