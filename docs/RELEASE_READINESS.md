...
- Source revision and CI: committed revision `665a9e646a2fa4514a29db8a3482c4b0e2bae499`; local release verification passed during the horizontal footer-badge deployment with 102 tests, production artifact scanning, and zero production dependency findings. This local commit has not been pushed and has no matching GitHub CI run; historical run `30062662075` applies only to earlier revision `dd38e44`
- Worker version: `9d34b374-2964-419e-aad9-eb54a64dc772`
...
- Live smoke: July 24, 2026, 18:23 Asia/Shanghai; canonical health reported `ok`, Worker `9d34b374-2964-419e-aad9-eb54a64dc772`, D1/R2, fresh maintenance, Kie.ai selected/configured, billing enabled, configured Stripe credentials/webhook/catalog, zero failed/stale events, zero open/unrecovered reviews, and healthy alerting. Canonical HTML loaded `/assets/index-DRLcCKZH.js` and `/assets/index-CAngVZFg.css`, exposed the horizontal responsive footer-badge group with both Findly.tools and Software Bolt, contained no obsolete vertical-group class or Startup Fame markup, and retained one canonical link. The response CSP continues to permit the exact `findly.tools` and `softwarebolt.com` image hosts; neither appears in `script-src` or `connect-src`. No canonical signed-in Checkout, payment, or provider request was used for this presentation deployment, so the prior Sandbox lifecycle and signed-in generation evidence remains authoritative. The Pages fallback was not redeployed.
