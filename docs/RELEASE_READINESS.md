...
| Automated tests | 99 passed, 0 failed, including direct successful-generation handoff to private Studio history, retained Studio download/favorite/variation/retry/delete controls, and no inline result galleries; Kie.ai task creation/polling/download, exact result-host enforcement, and unsupported-setting rejection; default-on GA4 page views with advertising signals disabled; account-required generation; exactly-once welcome credits; 4/8/16 credit charging for multi-tier providers; Starter-versus-Creator entitlements; shared three-plan homepage/full-pricing rendering with yearly default; dedicated footer trust routes and pre-release notice boundaries; explicit versioned policy acceptance; idempotent alert test delivery; alert deduplication/reminder/recovery; per-route self-referencing canonical metadata; unsupported-model rejection; stable non-JSON edge errors; concurrent D1 credits/idempotency; asynchronous Worker Stripe replay; out-of-order Radar fraud-warning retry/quarantine; authenticated and non-negative billing-review resolution; current Stripe Portal cancellation payloads; account-deletion billing tombstones; billing-event/review health degradation; maintenance repeatability; origin isolation; reviewed analytics CSP hosts; HTML transformation prevention; and external-policy contracts |
| Production bundle | Pass; JavaScript gzip about 102.5 KB; artifact scan rejects loopback addresses, SQLite customer copy, and development-token copy |
...
| Browser smoke | The current local Pages bundle registered a test account, generated a deterministic private preview, handed off directly to Studio history, saved the result as a favorite, created a second variation, and permanently removed that variation. Download, variation, favorite, and delete controls were visible in desktop and 390 px layouts with no horizontal overflow. Pricing, `/privacy`, `/status`, `/support`, mobile navigation, authentication, unauthenticated Studio, failure handling, and 404 retain earlier visual evidence. A prior signed-in Kie.ai generation completed the provider, credit, R2, browser-rendering, and authenticated-download path. |
...
| Cloudflare acceptance environment | Worker custom domains `qwen-image-3.net` and `www.qwen-image-3.net`, D1 database, private R2 bucket, Email Routing, and the 15-minute maintenance/alert trigger are deployed. Forward migrations `0001`–`0013` are applied. Committed revision `3022d25` runs as Worker `1a06f468-914d-454b-adad-a877a040c305`; health reports `ok`, Kie.ai selected/configured, configured/healthy external alerting, zero failed/stale billing events, zero open/unrecovered reviews, and a fresh scheduled check. The canonical and Pages fallback bundles retain the Studio-history handoff, authenticated download/favorite/variation/retry/delete controls, and no removed inline result/recent classes. Canonical public routes `/`, `/studio/history`, `/examples`, `/pricing`, `/privacy`, `/status`, and `/support` return HTTP 200; `www` permanently redirects to the apex. Legacy `/prompts` permanently redirects to `/examples`, and the live sitemap no longer includes `/prompts`. The Pages fallback was rebuilt as deployment `623d913e.qwen-image-3.pages.dev` with the modern catalog contract and deterministic local preview provider. Canonical `/terms` and `/refund-policy` expose approved policy version `2026-07-23`. GA4 loads on site entry without a prompt, advertising signals remain disabled, and Google's installation test detected the tag successfully. |
...
- Studio action and cleanup commit `532215a` plus Actions runtime commit `3022d25` are pushed on `codex/worker-online-hardening`;
- GitHub Actions run `30061188514` passed the full CI workflow with 99 tests and zero dependency-audit findings for pull request 1.
...
**Local policy and alert evidence:** `/pricing`, `/terms`, `/refund-policy`, and `/studio/billing` now expose the material renewal, cancellation, credit, refund, and review rules. Studio requires an explicit current-version checkbox; D1 records the acceptance and Checkout stores that version/time locally and in Stripe metadata. The exact approved copy and source digest are tracked in [Billing Terms Copy Approval](./BILLING_TERMS_APPROVAL.md). The aggregate alert route includes open and unrecovered reviews.
...
**Evidence:** baseline commit `39b9e3a` is pushed to `main` in `Tnsea/qwenimage3`. Draft pull request 1 tracks the hardening branch. Runtime revision `3022d25` passed GitHub Actions run `30061188514` with strict checks, 99 tests, Worker checking, the production build, local-artifact scanning, and zero production/full dependency-audit findings before deployment.
...
- Source revision and CI: committed revision `3022d254ed2ed5e169278477d18485c4d4cf7899`; immutable local `verify:release` passed with 99 tests and zero-finding production and full dependency audits; GitHub Actions run `30061188514` passed the full workflow
- Worker version: `1a06f468-914d-454b-adad-a877a040c305`
...
- Enabled providers: Kie.ai Qwen Image 2 with one externally successful Worker task, Google OAuth published for external Google accounts, and Cloudflare Email Routing billing alerts; Live Stripe credentials/webhook/catalog are configured but new Checkout remains disabled; deterministic local preview remains a development fallback, while Alibaba Qwen, GitHub, and transactional account email remain disabled
- Applied migrations: `0001`–`0013`; pre-change D1 export retained at ignored path `backups/qwen-image-3-20260723-155637.sql`
- Rollback identifiers: prior Worker `538f9a37-184b-466d-b897-ca5b98c5038c`; D1 restore has not been exercised
- Live smoke: July 24, 2026, 10:20 Asia/Shanghai; canonical health reported `ok`, Worker `1a06f468-914d-454b-adad-a877a040c305`, D1/R2, fresh maintenance, Kie.ai selected/configured, Google configured, Stripe credentials/webhook/catalog configured, sales disabled, configured/healthy alerting, and zero event/review counters. Canonical `/api/catalog` exposed only the configured `qwen2/text-to-image` model; `/`, `/studio/history`, `/examples`, `/pricing`, `/privacy`, `/status`, and `/support` returned HTTP 200; `www` returned HTTP 308 to the apex. The canonical, Pages fallback, and immutable Pages deployment loaded asset `/assets/index-DpcVjrYv.js`; each bundle contained the Studio download, variation, favorite, and permanent-delete controls and omitted the removed inline result/recent classes. The Pages fallback health and catalog exposed its explicitly deterministic `local-preview` provider and `local-qwen-preview` model. No new signed-in provider request was made for this UI-only deployment, so the prior fixed paid provider task `1c72e61aba62832596065a6d4b0aeb48` and signed-in product generation `9cc97bfb-5dfd-4d46-9815-d08ccec64510` remain the external evidence for Kie.ai execution, four-credit settlement, private R2 persistence, browser rendering, and authenticated download access. The immutable release test verified that an unsubscribed account receives a watermarked SVG, paid Starter receives the original PNG bytes with a `starter` export tier, and Creator receives the original bytes with a `vip` export tier; a live paid-Starter download remains unexercised while Checkout is disabled. The live status bundle contains the reviewed acceptance wording. The canonical policy routes matched the approved version. The operator alert test previously returned delivered, D1 recorded it once, Cloudflare Activity Log reported Delivered, and the next cron recorded a healthy check. Google's setup screen reported that the Google tag was detected correctly on the canonical site. Historical Google acceptance additionally covers real authorization, identity mapping, browser session, one-time starter grant, private Workspace load, and single-use state consumption.
...
The Worker and client runtime content are represented by committed revision `3022d254ed2ed5e169278477d18485c4d4cf7899` and were deployed only after immutable local release verification and a passing GitHub CI run. This record documents acceptance evidence only—not a Ready decision.
